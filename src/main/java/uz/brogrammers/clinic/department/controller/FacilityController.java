...
public class FacilityController {
    ...
    public ResponseEntity<?>addFacility(@RequestBody RegisterFacilityDto request){
        ...
        var facilityWithSameExists=department.getFacilities().stream()
                .filter(facility -> facility.getName().equals(request.getName()))
                .findAny();

        if (facilityWithSameExists.isPresent()){
            return  ResponseEntity.badRequest().body(new ApiResponse(false,"Bu hizmat bo'limi topilmadi",null));

        }
        var newFacility= Facility.builder()
                .name(request.getName())
                .analysisSet(new TreeSet<>())
                .build();
