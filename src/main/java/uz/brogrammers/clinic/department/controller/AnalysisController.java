...
public class AnalysisController {
    ...
    public ResponseEntity<?> addAnalysis(@RequestBody RegisterAnalysisRequest request) {
        ...
        if (!optionalDepartment.isPresent()) {
            return new ResponseEntity<>(new ApiResponse(false, "Bo'lim topilmadi", null),
        ...
        if (!optionalFacility.isPresent()) {
            return new ResponseEntity<>(new ApiResponse(false, "Xizmat topilmadi", null),
                    HttpStatus.BAD_REQUEST);
        }

        var facility = optionalFacility.get();

        var analysisWithSameNameExits = facility.getAnalysisSet().stream()
                .filter(analysis -> analysis.getName().equals(request.getName()))
                .findAny()
                .orElse(null);

        if (analysisWithSameNameExits != null) {
            return new ResponseEntity<>(new ApiResponse(false, "Bu analiz xizmatda topildi !", null),
                    HttpStatus.BAD_REQUEST);
        }
        ...
        var newAnalysis = Analysis.builder()
                ...
                .measurment(request.getMeasurements())
                .build();

        var savedAnalysis = analysisService.save(newAnalysis);
        facility.getAnalysisSet().add(savedAnalysis);
