...
public class UserController {
    ...
    @GetMapping("/me")
    public UserResponse findByUsername(@RequestParam(name = "username")
                                       String username){
    ...
    public List<UserResponse> findAll(){
        ...
        return  UserMapper.userToUserResponses(userList);
    }
    ...
    public ResponseEntity<?> register(@RequestBody UserRegistrationRequest userRegistrationRequest){
        if (userService.findByUsername
                (userRegistrationRequest.getUsername()).isPresent()){
            return new ResponseEntity<>(new ApiResponse(false,
                    "UserName already exists!",null),HttpStatus.CONFLICT);
